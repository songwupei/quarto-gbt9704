...
% 通过 \definefontfamily 体系定义黑体（ss 通道），确保 CJK 字体正确嵌入 PDF。
% \definefont 独立定义在 ConTeXt heading 中会导致字体未嵌入（PDF 乱码）。
% ==========================================================
\definefontfamily[mainface][ss][simhei][features=default]
\setupbodyfont[mainface, 16pt]  % 刷新以加载 ss（黑体）通道
...
% section 一级标题由模块 gbt9704-headings 通过 \gbt@@sectionfont 管理
% 不在此覆盖——ConTeXt 存在 \definefont 字体 ID 污染 bug，
% 样式覆盖无法解决黑体未嵌入 PDF 的问题，修复见上方 \definefontfamily + 下方 \SimHei 预初始化
...
% 预初始化 SimHei（黑体）—— 修复 ConTeXt 字体 ID 污染：
% 若在其他 \definefont 独立字体（如 \DaBiaoSong）之后才首次使用
% \SimHei，其字体 ID 被错误映射，导致标题乱码且字体未嵌入 PDF。
% 此命令必须在所有正文内容之前执行。
{\SimHei\relax}
