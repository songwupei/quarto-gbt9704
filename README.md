...
不想依赖自动检测？在 YAML 头中设置 `title-type`：

```yaml
---
title-type: none     # 或 tongzhi / biaozhun / auto
---
```

同一段 Markdown，四种模式的渲染结果完全不同：
...
| 模式 | `一、总体进展` | `1. 资金模型` | `**1. 问题：** 答案` | 适用 |
|------|---------------|--------------|---------------------|------|
| **`none`** | 正文 | 正文 | `**粗体**` + 正文 | 考试试卷、普通文档 |
| **`tongzhi`** | **H1** | **H3** | **全部 H3（粗体）** | 通知、报告、请示 |
| **`biaozhun`** | 正文 | **H1** | 正文 | 标准、规范、指南 |
| **`auto`** | 自动检测（默认向后兼容，无编号→通知模式） | | | |

> **关键区别**：
> - `none`：什么也不做，完全信任你的 Markdown `#` `##` `###`
> - `tongzhi`：中文编号 `一、` `（一）` `1.` 全部自动转标题
> - `biaozhun`：数字编号 `1` `2.1` `3.1.2` 自动转标题
> - `auto`：扫描全文自动判断，通知/标准模式二选一
>
> **选错模式会怎样？** 比如试卷用 `tongzhi` → `**1. 问题：** 答案` 会被整段转为标题（全部粗体）。试卷请用 `none`。
